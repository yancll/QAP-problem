  ...
  while(conteo<50000){
    ...
    if(muta+crusar>0){
      ...
      if(alet1<=sal13[1]){
        ...
        if(conteo<n112|n114<conteo & conteo<n115){salida=rbind(salida,tem13[1,])}
      ...
      if(alet1<=sal13[2]){
        ...
        if(conteo<n112|n114<conteo & conteo<n115){
          salida=rbind(salida,tem13[2,])
...
# abcd2 <- parLapply(cl, 1:10,function(x) {qap_complejo(d,w)})
# #abcd2=acopla(abcd2)
...
tf=proc.time()
(tf-t0)[3]
save(abcd1,file = "D:/segundo cuatrimestre/ICO/Tarea 2/result/dat.RData")
...
save(abcd1, abcd2,file = "D:/segundo cuatrimestre/ICO/Tarea 2/result/dat.RData")
data=datos(3)
d=data$d
w=data$w
...
# abcd3=sol30(d,w)
save(abcd1, abcd2,abcd3,file = "D:/segundo cuatrimestre/ICO/Tarea 2/result/dat.RData")
data=datos(4)
d=data$d
w=data$w
cl <- makeCluster(detectCores()-2) # Crea un cluster con 4 núcleos
...
stopCluster(cl) # Detiene el cluster
# abcd4=sol30(d,w)
save(abcd1, abcd2,abcd3,abcd4,file = "D:/segundo cuatrimestre/ICO/Tarea 2/result/dat.RData")
data=datos(5)
d=data$d
w=data$w
cl <- makeCluster(detectCores()-2) # Crea un cluster con 4 núcleos
...
stopCluster(cl) # Detiene el cluster
# abcd5=sol30(d,w)
save(abcd1, abcd2,abcd3,abcd4,abcd5,file = "D:/segundo cuatrimestre/ICO/Tarea 2/result/dat.RData")
